...
可能原因：Vercel Insights 脚本 (`/_vercel/insights/script.js`) 在微信环境中触发安全策略或加载异常，导致页面渲染异常。

### 12.3 解决方案

1. **移除 Vercel Insights 和 track.js**：从所有 351 个 HTML 页面中移除可能导致问题的脚本
2. **重写微信检测脚本 v2**：增加多重防护机制

### 12.4 微信检测脚本 v2 验证机制

**3 重环境检测**：
- UA 检测（micromessenger/wechat/wxwork/qq）
- 环境对象检测（WeixinJSBridge/wx/__wxjs_environment）
- 综合评分判定（满足任一条件即确认）

**2 次重试机制**：
- DOM 未就绪时自动重试（间隔 300ms、600ms）
- 5 秒超时兜底触发
...
**5 层错误隔离**：
- 最外层 try-catch 包裹整个脚本
- 每个函数独立 try-catch
- 每个 DOM 操作前检查存在性
- Promise 链式 catch
- 复制功能三级降级（clipboard API → execCommand → 手动提示）

### 12.5 统计方案调整

| 功能 | 之前 | 现在 |
|------|------|------|
| 总 PV/UV | 不蒜子 + Vercel Analytics | 不蒜子（不变） |
| 详细数据 | Vercel 后台 | Vercel 后台（不变） |
| admin 仪表盘 | 演示数据 + 真实 PV/UV | 真实 PV/UV（不变） |
...
| v5.2 | 2026-07-20 | 修复微信白屏问题：重写微信检测脚本v2（3重检测+2次重试+内容自检），移除 Vercel Insights 和 track.js 脚本 |
