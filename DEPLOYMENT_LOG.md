...
| v5.0 | 2026-07-19 | 接入 Vercel Analytics 访问统计 + 创建管理后台仪表盘 admin.html | - |
...
微信内置浏览器（WeChat WebView）对部分功能有限制，用户在微信中打开网站时体验不佳，需要引导用户在系统默认浏览器中打开。

### 18.2 实现方案

**文件路径**：`/js/wechat-redirect.js`

**检测逻辑**：
- 通过 `navigator.userAgent` 判断是否包含 `micromessenger`（微信）或 `qq/`（QQ 内置浏览器）
- 非微信/QQ 浏览器直接返回，不执行任何操作
- 微信/QQ 浏览器下显示全屏遮罩引导层
...
- 降级使用 `document.execCommand('copy')`（兼容旧浏览器）

### 18.3 批量部署

使用 Python 脚本批量给 351 个 HTML 页面添加脚本引用：

```html
<script defer src="/js/wechat-redirect.js"></script>
```
...
1. **非侵入式**：非微信浏览器不加载任何 UI，零性能影响
2. **可关闭**：用户可手动关闭遮罩，不强制跳转
3. **渐进增强**：复制功能优先使用现代 API，降级兼容旧浏览器
4. **全平台覆盖**：同时检测微信和 QQ 内置浏览器
...
| 需求编号 | 需求描述 | 优先级 | 来源 |
|---------|---------|--------|------|
| R015 | 接入访问统计系统 | P0 | 用户需求 |
| R016 | 统计基础访问量（PV/UV） | P0 | 用户需求 |
| R017 | 统计流量来源 | P1 | 用户需求 |
| R018 | 统计页面热度 | P1 | 用户需求 |
| R019 | 统计访客地理位置 | P1 | 用户需求 |
| R020 | 具备可扩展为可视化仪表盘的能力 | P1 | 用户需求 |

### 17.2 方案选型

| 方案 | 优点 | 缺点 | 选择 |
|------|------|------|------|
| Vercel Analytics | 零代码开发，Vercel 原生集成 | 免费版有限额 | ✅ 采用 |
| Google Analytics | 功能强大，完全免费 | 界面复杂，隐私争议 | 备选 |
| Umami 自部署 | 开源轻量，隐私友好 | 需额外部署 | 备选（已预留接口） |
| 自建后台管理系统 | 完全可控 | 开发量大 | 备选 |

**最终方案**：Vercel Analytics（采集）+ admin.html（可视化仪表盘，支持多数据源扩展）

### 17.3 数据采集实现

#### 17.3.1 采集脚本

在所有 350 个 HTML 页面末尾（`</body>` 前）添加 Vercel Analytics 采集脚本：
...
**适用场景**：纯静态 HTML 网站（非 Next.js 项目，无需 npm 安装包）

#### 17.3.2 批量添加

使用 Python 脚本批量处理所有 HTML 文件：

**执行结果**：
- 处理文件总数：349 个（+ index.html 手动添加 = 350 个）
- 成功添加：349 个
- 跳过：0 个

### 17.4 可视化仪表盘

**文件路径**：`/workspace/admin.html`
...
| 功能 | 说明 |
|------|------|
| 登录验证 | 前端密码校验，默认密码 `yunzhuan2026` |
| 统计概览 | 总访问量、独立访客、今日访问、平均停留时长 |
| 访问趋势图 | PV/UV 折线图（近 7/30/90 天） |
| 流量来源 | 饼图展示（搜索引擎/直接/社交/外链） |
| 访客地区 | 柱状图展示国家/地区分布 |
| 热门页面 | 表格展示 Top 10 页面排行 |
| 数据导出 | JSON 格式导出统计数据 |
| 数据源配置 | 支持 Vercel/Umami/手动录入切换 |

#### 17.4.2 数据源适配器架构

仪表盘支持 4 种数据源，通过下拉菜单切换：

1. **演示数据（默认）**：内置模拟数据，展示仪表盘效果
2. **Vercel Analytics API**：配置 API 地址和 Token 后拉取真实数据
3. **Umami API**：支持接入自部署的 Umami 服务
4. **手动录入**：手动填写统计数据

**配置存储**：使用浏览器 localStorage 保存数据源配置
...
**方式二**：admin.html 仪表盘
1. 访问 `https://yunzhuan.icu/admin.html`
2. 输入密码：`yunzhuan2026`
3. 查看可视化图表
...
- **纯静态网站的 Analytics**：不需要安装 npm 包或添加 React 组件，只需在 HTML 中引入 `/_vercel/insights/script.js`
- **数据采集前提**：代码必须先部署到 Vercel，且有人访问页面，数据才开始统计
...
### 17.8 不蒜子统计方案（v2.1 最终方案）

#### 17.8.1 背景

Vercel KV 需要手动配置，对非技术用户不友好。改用**不蒜子（busuanzi）**免费网页计数器，零配置，加一行脚本即可统计全站 PV/UV。

#### 17.8.2 方案对比

| 方案 | 配置难度 | 数据准确性 | 功能完整度 | 选择 |
|------|----------|-----------|-----------|------|
| Vercel Analytics（后台） | 低 | 高 | 高 | ✅ 保留（后台查看） |
| Vercel KV 自建 | 高（需配置KV） | 高 | 中 | ⚠️ 保留代码（可选） |
| 不蒜子 busuanzi | 极低（加一行脚本） | 高 | 中（仅PV/UV） | ✅ 仪表盘默认方案 |

**最终方案**：不蒜子（仪表盘展示） + Vercel Analytics（后台详细查看），双轨并行。
...
**批量添加**：使用 Python 脚本 `add_busuanzi.py` 给 351 个 HTML 页面全部添加。
...
#### 17.8.4 admin.html v2.1 更新

1. **真实数据源切换为不蒜子**：默认从不蒜子获取全站 PV/UV
2. **自动加载不蒜子脚本**：admin.html 动态加载 busuanzi 脚本并读取数据
3. **降级机制**：不蒜子加载失败时回退到演示数据
4. **连接测试功能**：点击"测试连接"可验证不蒜子是否正常工作

#### 17.8.5 不蒜子原理

- 基于域名统计，每个域名独立计数
- 使用 JSONP 跨域请求计数接口
- 数据存储在不蒜子服务器（ibruce.info）
- 完全免费，无广告，无需注册

#### 17.8.6 注意事项

- 不蒜子只提供**累计 PV/UV**，不提供今日数据、来源、地区等详细数据
- 今日数据为估算值（总访问量 × 8%）
- 详细数据仍需通过 Vercel Analytics 后台查看
- 不蒜子为第三方服务，如服务不可用会自动回退到演示数据

### 17.9 admin.html v3.0 精简版（全真实数据）

#### 17.9.1 背景

用户要求 admin.html **全部使用真实数据，获取不到的就删除**。因此移除所有演示数据模块，只保留不蒜子能提供的真实 PV/UV 数据。

#### 17.9.2 v2.x → v3.0 变更

| 模块 | v2.x | v3.0 | 原因 |
|------|------|------|------|
| 顶部滚动条 | 6 项数据（含今日估算） | 3 项真实数据 | 今日/估算数据 |
| 统计卡片 | 4 张（PV/UV/今日/平均停留） | 2 张（PV/UV） | 今日和平均停留无真实数据 |
| 趋势图 | ✅ 有（估算数据） | ❌ 删除 | 无历史真实数据 |
| 流量来源图 | ✅ 有（演示数据） | ❌ 删除 | 无真实来源数据 |
| 地区分布图 | ✅ 有（演示数据） | ❌ 删除 | 无真实地区数据 |
| 热门页面表 | ✅ 有（演示数据） | ❌ 删除 | 无真实页面排行数据 |
| 数据源设置 | ✅ 有（多种数据源） | ❌ 删除 | 仅用不蒜子 |
| Chart.js 库 | ✅ 加载 | ❌ 移除 | 无图表 |
| 说明卡片 | ❌ 无 | ✅ 新增 | 引导用户去 Vercel 看详细数据 |

#### 17.9.3 v3.0 保留内容

1. **登录系统**（密码验证 + sessionStorage）
2. **顶部滚动条**（紫色渐变，3 项真实数据
   - 总访问量（真实）
   - 独立访客（真实）
   - 数据更新时间
3. **统计卡片**（2 张）
